...
/** 二值化后用于OLED显示的数据 */
//unsigned char imagin_cam[LCDH][LCDW];
...
//    CAMERA_Init(50);
    ...
    while (1)
    {
       ...
       //     Get_Use_Image();
            ...
            // OLED动态显示摄像头图像
            OLED_Road(LCDH, LCDW, (unsigned char *)imagin_cam);
#else
            ...
            // 显示摄像头图像
            TFTSPI_BinRoad(0, 0, LCDH, LCDW, (uint8*) imagin_cam);
#endif
#endif
#endif
          //  LED_Ctrl(LED0, RVS);
...
void CAMERA_Reprot (void)
{
    ...
    for(i=0 ;i<120;i++)
    {
        Image_Data[i][93] = 0xf0;
        Image_Data[i][94] = 0xf0;
    }
    for (i = 0; i < IMAGEH; i++)
    {
        for (j = 0; j < IMAGEW; j++)
...
void Get_Bin_Image(unsigned char mode)
{
    ...
    {
        Threshold = 80;                          //手动调节阈值
  //      lq_sobel(Image_Use, imagin_cam, (unsigned char) Threshold);
    ...
    {
 //       lq_sobelAutoThreshold(Image_Use, imagin_cam);  //动态调节阈值
        return;
    }
/*
#ifdef USEOLED
 ...
 * @date     2020/5/15
 */
void lq_sobel (unsigned char imageIn[LCDH][LCDW], _Bool imageOut[LCDH][LCDW], unsigned char Threshold)
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
 ...
 * @date     2020/5/15
 */
void lq_sobelAutoThreshold (unsigned char imageIn[LCDH][LCDW], _Bool imageOut[LCDH][LCDW])
{
    /** 卷积核大小 */
    short KERNEL_SIZE = 3;
    short xStart = KERNEL_SIZE / 2;
...
/*---------------------------------------------------------------
 【函    数】imagin_cam_Filter void Bin_Image_Filter(void)
 【功    能】过滤噪点
 【参    数】无
 【返 回 值】无
 【注意事项】
...
void Bin_Image_Filter(void)
{
    ...
    for (nr = 1; nr < LCDH - 1; nr++)
    {
        ...
        {
            if ((imagin_cam[nr][nc] == 0)
                    && (imagin_cam[nr - 1][nc] + imagin_cam[nr + 1][nc] + imagin_cam[nr][nc + 1] + imagin_cam[nr][nc - 1] > 2))
            {
                imagin_cam[nr][nc] = 1;
            }
            else if ((imagin_cam[nr][nc] == 1)
                    && (imagin_cam[nr - 1][nc] + imagin_cam[nr + 1][nc] + imagin_cam[nr][nc + 1] + imagin_cam[nr][nc - 1] < 2))
            {
                imagin_cam[nr][nc] = 0;
            }
